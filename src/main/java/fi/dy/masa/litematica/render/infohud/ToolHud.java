    ...
    protected void updateHudText() {
        ...
        if (hasTool && DataManager.getSchematicProjectsManager().hasProjectOpen()) {
            ...
            lines.add(StringUtils.translate(
                    "litematica.hud.schematic_projects.project_name", green + project.getName() + rst));
            SchematicVersion version = project.getCurrentVersion();

            if (version != null) {
                lines.add(StringUtils.translate(
                        "litematica.hud.schematic_projects.current_version",
                        green + version.getVersion() + rst,
                        green + project.getVersionCount() + rst,
                        green + version.getName() + rst));
                DATE.setTime(version.getTimeStamp());
                lines.add(StringUtils.translate(
                        "litematica.hud.schematic_projects.current_version_date",
                        green + SIMPLE_DATE_FORMAT.format(DATE) + rst));
            ...
            if (selection != null && sm.getSelectionMode() == SelectionMode.NORMAL) {
                ...
                if (subRegionName != null) {
                    lines.add(StringUtils.translate(
                            "litematica.hud.area_selection.selected_sub_region", green + subRegionName + rst));
                }
            }
        ...
        if (hasTool && mode == ToolMode.DELETE) {
            String strp = ToolModeData.DELETE.getUsePlacement()
                    ? "litematica.hud.delete.target_mode.placement"
                    : "litematica.hud.delete.target_mode.area";
            lines.add(StringUtils.translate(
                    "litematica.hud.delete.target_mode", green + StringUtils.translate(strp) + rst));
        }
        ...
        if (hasTool && mode.getUsesAreaSelection()) {
            ...
            if (selection != null) {
                ...
                if (subRegionName != null && box != null) {
                    ...
                    if (p1 != null && p2 != null) {
                        ...
                        lines.add(StringUtils.translate(
                                "litematica.hud.area_selection.dimensions_position", strDim, strp1, strp2));
                    }
                }
            }
            ...
            if (schematicPlacement != null) {
                ...
                if (state != null
                        && mode == ToolMode.REBUILD
                        && (stack.isEmpty()
                                || EntityUtils.hasToolItemInHand(this.mc.player, InteractionHand.MAIN_HAND))) {
                    lines.add(StringUtils.translate("litematica.tool_hud.block_1", this.getBlockString(state)));
                }
                ...
                if (placement != null) {
                    ...
                    or = placement.getPos();
                    or = PositionUtils.getTransformedBlockPos(
                            or, schematicPlacement.getMirror(), schematicPlacement.getRotation());
                    or = or.offset(schematicPlacement.getOrigin());
                    str = String.format("%d, %d, %d", or.getX(), or.getY(), or.getZ());
                    lines.add(StringUtils.translate(
                            "litematica.hud.schematic_placement.sub_region_origin", green + str + rst));
                }

                if (mode == ToolMode.PASTE_SCHEMATIC) {
                    ReplaceBehavior replace =
                            (ReplaceBehavior) Configs.Generic.PASTE_REPLACE_BEHAVIOR.getOptionListValue();
                    ...
                    str = Configs.Generic.PASTE_NBT_BEHAVIOR
                            .getOptionListValue()
                            .getDisplayName();

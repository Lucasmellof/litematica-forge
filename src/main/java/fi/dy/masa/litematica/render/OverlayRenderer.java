...
public class OverlayRenderer {
    ...
    public void renderBoxes(PoseStack matrices) {
        ...
        boolean renderAreas =
                currentSelection != null && Configs.Visuals.ENABLE_AREA_SELECTION_RENDERING.getBooleanValue();
        boolean renderPlacements =
                !this.placements.isEmpty() && Configs.Visuals.ENABLE_PLACEMENT_BOXES_RENDERING.getBooleanValue();
        ...
        if (renderAreas || renderPlacements || isProjectMode) {
            ...
            if (renderAreas) {
                ...
                if (origin != null) {
                    ...
                    Color4f color =
                            currentSelection.isOriginSelected() ? this.colorSelectedCorner : this.colorAreaOrigin;
            ...
            if (renderPlacements) {
                ...
                for (Map.Entry<SchematicPlacement, ImmutableMap<String, Box>> entry : this.placements.entrySet()) {
                    ...
                    for (Map.Entry<String, Box> entryBox : boxMap.entrySet()) {
                        ...
                        boolean boxSelected = schematicPlacement == currentPlacement
                                && (origin || boxName.equals(schematicPlacement.getSelectedSubRegionName()));
                        BoxType type = boxSelected ? BoxType.PLACEMENT_SELECTED : BoxType.PLACEMENT_UNSELECTED;
                        this.renderSelectionBox(
                                entryBox.getValue(), type, expand, 1f, 1f, schematicPlacement, matrices);
                    }

                    Color4f color = schematicPlacement == currentPlacement && origin
                            ? this.colorSelectedCorner
                            : schematicPlacement.getBoxesBBColor();
                    RenderUtils.renderBlockOutline(
                            schematicPlacement.getOrigin(), expand, lineWidthBlockBox, color, this.mc);
                    ...
                    if (Configs.Visuals.RENDER_PLACEMENT_ENCLOSING_BOX.getBooleanValue()) {
                        ...
                        if (schematicPlacement.shouldRenderEnclosingBox() && box != null) {
                            RenderUtils.renderAreaOutline(
                                    box.getPos1(), box.getPos2(), 1f, color, color, color, this.mc);
    ...
    public void renderSelectionBox(
        ...
        if (pos1 != null && pos2 != null) {
            if (!pos1.equals(pos2)) {
                ...
                RenderUtils.renderBlockOutlineOverlapping(
                        pos1, expand, lineWidthBlockBox, color1, color2, this.colorOverlapping, matrices, this.mc);
            }
        } else {
            if (pos1 != null) {
                RenderUtils.renderBlockOutline(pos1, expand, lineWidthBlockBox, color1, this.mc);
    ...
    public void renderSchematicVerifierMismatches(PoseStack matrices) {
        ...
        if (placement != null && placement.hasVerifier()) {
            ...
            if (!list.isEmpty()) {
                ...
                BlockPos posLook =
                        trace != null && trace.getType() == HitResult.Type.BLOCK ? trace.getBlockPos() : null;
                this.renderSchematicMismatches(list, posLook, matrices);
            }
        }
    }

    private void renderSchematicMismatches(
            List<MismatchRenderPos> posList, @Nullable BlockPos lookPos, PoseStack matrices) {
        ...
        if (lookedEntry != null) {
            if (connections && prevEntry != null) {
                RenderUtils.drawConnectingLineBatchedLines(
                        prevEntry.pos, lookedEntry.pos, false, lookedEntry.type.getColor(), buffer, this.mc);
            }
            ...
            RenderUtils.drawBlockBoundingBoxOutlinesBatchedLines(
                    lookPos, lookedEntry.type.getColor(), 0.002, buffer, this.mc);
        }
    ...
    public void renderHoverInfo(Minecraft mc, PoseStack matrixStack) {
        if (mc.level != null && mc.player != null) {
            ...
            boolean renderBlockInfoOverlay = !verifierOverlayRendered
                    && infoOverlayKeyActive
    ...
    private void renderBlockInfoLines(RayTraceWrapper traceWrapper, Minecraft mc, PoseStack matrixStack) {
        ...
        fi.dy.masa.malilib.render.RenderUtils.renderText(
                x,
                y,
                fontScale,
                textColor,
            ...
            if (trace != null && trace.getType() == HitResult.Type.BLOCK) {
                ...
                if (mismatch != null && worldSchematic != null) {
                    ...
                    BlockInfoAlignment align =
                            (BlockInfoAlignment) Configs.InfoOverlays.BLOCK_INFO_OVERLAY_ALIGNMENT.getOptionListValue();
    ...
    protected void getOverlayPosition(
            BlockInfoAlignment align, int width, int height, int offY, int invHeight, Minecraft mc) {
        switch (align) {
            case CENTER:

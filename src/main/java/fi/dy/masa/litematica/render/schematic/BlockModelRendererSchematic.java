...
public class BlockModelRendererSchematic {
    ...
    public boolean renderModel(
            ...
            long rand) {
        boolean ao =
                Minecraft.useAmbientOcclusion() && stateIn.getLightEmission() == 0 && modelIn.useAmbientOcclusion();
        ...
        try {
            if (ao) {
                return this.renderModelSmooth(
                        worldIn, modelIn, stateIn, posIn, matrices, vertexConsumer, this.random, rand, overlay);
            } else {
    ...
    public boolean renderModelSmooth(
        ...
        if (!quads.isEmpty()) {
            this.renderQuadsSmooth(
                    worldIn, stateIn, posIn, matrices, vertexConsumer, quads, quadBounds, bitset, aoFace, overlay);
            renderedSomething = true;
        }
            ...
            if (!quads.isEmpty()) {
                if (this.shouldRenderModelSide(worldIn, stateIn, posIn, side)) {
                    ...
                    this.renderQuadsFlat(
                            worldIn, stateIn, posIn, light, overlay, false, matrices, vertexConsumer, quads, bitset);
                    renderedSomething = true;
                }
            }
        }
    ...
    private boolean shouldRenderModelSide(
            BlockAndTintGetter worldIn, BlockState stateIn, BlockPos posIn, Direction side) {
    ...
    private void renderQuadsSmooth(
        ...
        for (BakedQuad bakedQuad : list) {
            ...
            ambientOcclusionCalculator.apply(
                    world, state, pos, bakedQuad.getDirection(), box, flags, bakedQuad.isShade());
    ...
    private void renderQuadsFlat(
        ...
        for (BakedQuad bakedQuad : list) {
            if (useWorldLight) {
                this.getQuadDimensions(
                        world, state, pos, bakedQuad.getVertices(), bakedQuad.getDirection(), null, flags);
    ...
    private void getQuadDimensions(
        ...
        boolean b = minX >= min || minY >= min || maxX <= max || maxY <= max;
        boolean c = minX >= min || minZ >= min || maxX <= max || maxZ <= max;
        boolean d = minY >= min || minZ >= min || maxY <= max || maxZ <= max;
        switch (face) {
            case DOWN:
                flags.set(1, c);
    ...
    class AmbientOcclusionCalculator {
        ...
        private int getVertexBrightness(
            ...
            int i = (int) ((float) (p_178203_1_ >> 16 & 255) * p_178203_5_
                    ...
                    & 255;
            int j = (int) ((float) (p_178203_1_ & 255) * p_178203_5_

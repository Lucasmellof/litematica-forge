...
public class RayTraceUtils {
    private static final net.minecraft.world.phys.AABB FULL_BLOCK_BOUNDS =
    ...
    public static RayTraceWrapper getWrappedRayTraceFromEntity(Level world, Entity entity, double range) {
        ...
        double closestVanilla =
                result.getType() != HitResult.Type.MISS ? result.getLocation().distanceTo(eyesPos) : -1D;
    ...
    private static boolean traceToOrigin(
            BlockPos pos, Vec3 start, Vec3 end, HitType type, @Nullable SchematicPlacement placement) {
        if (pos != null) {
            net.minecraft.world.phys.AABB bb = PositionUtils.createAABBForPosition(pos);
            Optional<Vec3> optional = bb.clip(start, end);
    ...
    public static BlockHitResult traceToPositions(List<BlockPos> posList, Entity entity, double range) {
        ...
        for (BlockPos pos : posList) {
            if (pos != null) {
                BlockHitResult hit = net.minecraft.world.phys.AABB.clip(
                        ImmutableList.of(FULL_BLOCK_BOUNDS), eyesPos, lookEndPos, pos);
    ...
    @Nullable
    public static BlockHitResult traceToSchematicWorld(
            Entity entity, double range, boolean respectRenderRange, boolean targetFluids) {
    ...
    @Nullable
    public static RayTraceWrapper getGenericTrace(
            Level worldClient,
            Entity entity,
            double range,
            boolean respectRenderRange,
            boolean targetFluids,
            boolean includeVerifier) {
    ...
    @Nullable
    public static RayTraceWrapper getSchematicWorldTraceWrapperIfClosest(
            Level worldClient, Entity entity, double range) {
    ...
    public static BlockPos getSchematicWorldTraceIfClosest(Level worldClient, Entity entity, double range) {
        ...
        return trace != null && trace.getHitType() == HitType.SCHEMATIC_BLOCK
                ? trace.getBlockHitResult().getBlockPos()
                : null;
    }

    @Nullable
    public static BlockPos getFurthestSchematicWorldBlockBeforeVanilla(
            Level worldClient, Entity entity, double maxRange, boolean requireVanillaBlockBehind) {
        Vec3 eyesPos = entity.getEyePosition(1f);
        ...
        List<BlockHitResult> list = rayTraceBlocksToList(
                worldSchematic, eyesPos, lookEndPos, ClipContext.Fluid.NONE, false, false, true, 200);
    ...
    @Nullable
    public static RayTraceWrapper getFurthestSchematicWorldTraceBeforeVanilla(
            Level worldClient, Entity entity, double maxRange) {
        Vec3 eyesPos = entity.getEyePosition(1f);
        Vec3 rangedLookRot = entity.getViewVector(1f).scale(maxRange);
        Vec3 lookEndPos = eyesPos.add(rangedLookRot);
     ...
     */
    @Nullable
    public static BlockHitResult rayTraceBlocks(
            Level world,
            Vec3 start,
            Vec3 end,
        ...
        BlockHitResult trace = traceFirstStep(
                data,
                world,
                blockState,
                fluidState,
    ...
    @Nullable
    private static BlockHitResult traceFirstStep(
    ...
    @Nullable
    private static boolean traceLoopSteps(
            RayTraceCalcsData data,
            Level world,
            BlockState blockState,
            FluidState fluidState,
        ...
        if ((!respectLayerRange || data.range.isPositionWithinRange(data.x, data.y, data.z))
                && (!ignoreBlockWithoutBoundingBox
                        || blockState.getMaterial() == Material.PORTAL
                        || !blockState.getCollisionShape(world, data.blockPos).isEmpty())) {
            VoxelShape blockShape = blockState.getShape(world, data.blockPos);
            boolean blockCollidable = !blockShape.isEmpty();
            boolean fluidCollidable = data.fluidMode.canPick(fluidState);
    ...
    public static List<BlockHitResult> rayTraceBlocksToList(
            Level world,
            Vec3 start,
            Vec3 end,
            ClipContext.Fluid fluidMode,
    ...
    private static boolean rayTraceCalcs(
            RayTraceCalcsData data, boolean returnLastNonCollidableBlock, boolean respectLayerRange) {

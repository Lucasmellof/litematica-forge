...
public class SchematicWorldRefresher implements IRangeChangeListener {
    ...
    public void updateBetweenX(int minX, int maxX) {
        ...
        if (world != null && this.mc.level != null) {
            ...
            final int minY = world.getMinBuildHeight();
            final int maxY = world.getMaxBuildHeight() - 1;
            Long2ObjectMap<ChunkSchematic> schematicChunks =
            ...
            for (ChunkSchematic chunk : schematicChunks.values()) {
                ...
                if (pos.x >= cxMin
                    ...
                    world.scheduleChunkRenders(minX, minY, (pos.z << 4), maxX, maxY, (pos.z << 4) + 15);
                }
            }
        }
    }
    ...
    public void updateBetweenY(int minY, int maxY) {
        ...
        if (world != null && this.mc.level != null) {
            ...
            for (ChunkSchematic chunk : schematicChunks.values()) {
                ...
                // Only mark chunks that are actually rendered (if the schematic world contains more chunks)
                if (!chunk.isEmpty() && WorldUtils.isClientChunkLoaded(this.mc.level, pos.x, pos.z)) {
                    world.scheduleChunkRenders(
                            (pos.x << 4), minY, (pos.z << 4), (pos.x << 4) + 15, maxY, (pos.z << 4) + 15);
                }
            }
        }
    }
        ...
        if (world != null && this.mc.level != null) {
            ...
            for (ChunkSchematic chunk : schematicChunks.values()) {
                ...
                if (pos.z >= czMin
                    ...
                    world.scheduleChunkRenders((pos.x << 4), minY, minZ, (pos.x << 4) + 15, maxY, maxZ);
                }
            }
        }
    }

...
public class InventoryUtils {
    ...
    public static void setPickBlockableSlots(String configStr) {
        ...
        for (String str : parts) {
            try {
                ...
                if (Inventory.isHotbarSlot(slotNum) && !PICK_BLOCKABLE_SLOTS.contains(slotNum)) {
        ...
        if (Inventory.isHotbarSlot(sourceSlot)) {
            ...
            if (PICK_BLOCKABLE_SLOTS.size() == 0) {
                InfoUtils.showGuiOrInGameMessage(
                        MessageType.WARNING, "litematica.message.warn.pickblock.no_valid_slots_configured");
                return;
            }
            ...
            if (hotbarSlot != -1) {
                ...
                InfoUtils.showGuiOrInGameMessage(
                        MessageType.WARNING, "litematica.message.warn.pickblock.no_suitable_slot_found");
            }
        }
    }
        ...
        if (!stack.isEmpty()) {
            ...
            if (EntityUtils.isCreativeMode(mc.player)) {
                ...
                mc.gameMode.handleCreativeModeItemAdd(
                        mc.player.getItemInHand(InteractionHand.MAIN_HAND), 36 + inv.selected);
                ...
                if (shouldPick && slot != -1) {
                    ...
                    if (slot != -1) {
                        ItemStack boxStack =
                                mc.player.inventoryMenu.slots.get(slot).getItem();
    ...
    public static int findSlotWithBoxWithItem(
            AbstractContainerMenu container, ItemStack stackReference, boolean reverse) {
        ...
        for (int slotNum = startSlot; slotNum != endSlot; slotNum += increment) {
            ...
            if ((!isPlayerInv
                            || fi.dy.masa.malilib.util.InventoryUtils.isRegularInventorySlot(slot.index, false))

...
public class InputHandler implements IKeybindProvider, IKeyboardInputHandler, IMouseInputHandler {
    ...
    public void addHotkeys(IKeybindManager manager) {
        manager.addHotkeysForCategory(
                Reference.MOD_NAME, "litematica.hotkeys.category.generic_hotkeys", Hotkeys.HOTKEY_LIST);
    }
    ...
    public boolean onKeyInput(int keyCode, int scanCode, int modifiers, boolean eventKeyState) {
        if (eventKeyState) {
            ...
            } else if (mc.options.keyScreenshot.matches(keyCode, scanCode)
                    && GuiSchematicManager.hasPendingPreviewTask()) {
        ...
        if (Hotkeys.SELECTION_GRAB_MODIFIER.getKeybind().isKeybindHeld()) {
            if (mode.getUsesAreaSelection()) {
                ...
                if (sm.hasGrabbedElement()) {
                    ...
                    area.moveEntireSelectionTo(
                            old.relative(EntityUtils.getClosestLookingDirection(entity), amount), false);
                    return true;
    ...
    private boolean handleAttackKey(Minecraft mc) {
        if (mc.player != null
                ...
                && KeybindMulti.getTriggeredCount() == 0) {

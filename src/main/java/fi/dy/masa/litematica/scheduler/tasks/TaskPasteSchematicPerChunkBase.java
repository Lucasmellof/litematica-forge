...
public abstract class TaskPasteSchematicPerChunkBase extends TaskProcessChunkMultiPhase {
    ...
    public TaskPasteSchematicPerChunkBase(
    ...
    protected void addPlacement(SchematicPlacement placement, LayerRange range) {
        ...
        for (ChunkPos pos : touchedChunks) {
            ...
            for (IntBoundingBox box :
                    placement.getBoxesWithinChunk(pos.x, pos.z).values()) {
    ...
    protected boolean canProcessChunk(ChunkPos pos) {
        if (!this.schematicWorld.getChunkProvider().hasChunk(pos.x, pos.z)
                || DataManager.getSchematicPlacementManager().hasPendingRebuildFor(pos)) {
            return false;
        }

        // Chunk exists in the schematic world, and all the surrounding chunks are loaded in the client world, good to
        // go

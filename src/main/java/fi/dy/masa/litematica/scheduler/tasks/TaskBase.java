...
public abstract class TaskBase implements ITask, IInfoHudRenderer {
    ...
    protected void updateInfoHudLinesPendingChunks(Collection<ChunkPos> pendingChunks) {
        ...
        if (!pendingChunks.isEmpty()) {
            ...
            PositionUtils.CHUNK_POS_COMPARATOR.setReferencePosition(new BlockPos(this.mc.player.position()));
            PositionUtils.CHUNK_POS_COMPARATOR.setClosestFirst(true);
            list.sort(PositionUtils.CHUNK_POS_COMPARATOR);

            String pre = GuiBase.TXT_WHITE + GuiBase.TXT_BOLD;
            String title = StringUtils.translate(
                    "litematica.gui.label.task.title.remaining_chunks", this.getDisplayName(), pendingChunks.size());
            ...
            for (int i = 0; i < maxLines; ++i) {
                ...
                this.infoHudLines.add(
                        String.format("cx: %5d, cz: %5d (x: %d, z: %d)", pos.x, pos.z, pos.x << 4, pos.z << 4));
            }
        }
    }

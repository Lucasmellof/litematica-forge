...
public class SelectionManager {
        ...
        if (DataManager.getSchematicProjectsManager().hasProjectOpen()) {
            ...
            if (project != null) {
                ...
                InfoUtils.showGuiOrInGameMessage(
                        MessageType.WARNING,
                        "litematica.error.schematic_projects.in_projects_mode_but_no_project_open");
            }
        } else {
    ...
    public boolean renameSelection(
            File dir, String selectionId, String newName, boolean copy, IMessageConsumer feedback) {
        File file = new File(selectionId);
        ...
        if (file.exists() && file.isFile()) {
            ...
            if (newFileName.isEmpty()) {
                feedback.addMessage(
                        MessageType.ERROR,
                        "litematica.error.area_selection.rename.invalid_safe_file_name",
                        newFileName);
                return false;
            }
            ...
            if (newFile.exists() == false && (copy || file.renameTo(newFile))) {
                ...
                feedback.addMessage(
                        MessageType.ERROR, "litematica.error.area_selection.rename.already_exists", newFile.getName());
            }
        }
            ...
            if (selection.createNewSubRegionBox(pos, selection.getName()) != null) {
                if (printMessage) {
                    ...
                    InfoUtils.showGuiOrActionBarMessage(
                            MessageType.SUCCESS, "litematica.message.added_selection_box", posStr);
                }
    ...
    public boolean createSelectionFromPlacement(
            File dir, SchematicPlacement placement, String name, IMessageConsumer feedback) {
        String safeName = FileUtils.generateSafeFileName(name);

        if (safeName.isEmpty()) {
            feedback.addMessage(
                    MessageType.ERROR, "litematica.error.area_selection.rename.invalid_safe_file_name", safeName);
            return false;
        }
    ...
    public boolean changeSelection(Level world, Entity entity, int maxDistance) {
        ...
        if (area != null) {
            ...
            if (trace.getHitType() == HitType.SELECTION_BOX_CORNER
                    || trace.getHitType() == HitType.SELECTION_BOX_BODY
                    || trace.getHitType() == HitType.SELECTION_ORIGIN) {
    ...
    public void setPositionOfCurrentSelectionToRayTrace(
            Minecraft mc, Corner corner, boolean moveEntireSelection, double maxDistance) {
        AreaSelection area = this.getCurrentSelection();
    ...
    public void moveSelectionOrigin(AreaSelection area, BlockPos newOrigin, boolean moveEntireSelection) {
        if (moveEntireSelection) {
            ...
            String posStrNew =
                    String.format("x: %d, y: %d, z: %d", newOrigin.getX(), newOrigin.getY(), newOrigin.getZ());
            InfoUtils.showGuiOrActionBarMessage(
                    MessageType.SUCCESS, "litematica.message.moved_area_origin", posStrOld, posStrNew);
        }
    }

    public void handleCuboidModeMouseClick(
            Minecraft mc, double maxDistance, boolean isRightClick, boolean moveEntireSelection) {
    ...
    public GuiBase getEditGui() {
        ...
        if (this.getSelectionMode() == SelectionMode.NORMAL) {
            if (selection != null) {
                ...
                InfoUtils.showGuiOrActionBarMessage(
                        MessageType.WARNING, "litematica.error.area_editor.open_gui.no_selection");
                return null;
            }
        } else {

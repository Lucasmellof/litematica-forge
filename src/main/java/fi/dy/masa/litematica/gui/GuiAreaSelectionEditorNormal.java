...
public class GuiAreaSelectionEditorNormal
    ...
    public void initGui() {
        ...
        if (this.selection != null) {
            ...
            this.addLabel(
                    20, 30, 120, 12, 0xFFFFAA00, StringUtils.translate("litematica.error.area_editor.no_selection"));
        }
    }
    ...
    protected void createSelectionEditFields() {
        ...
        this.addLabel(
                x, y, -1, 16, 0xFFFFFFFF, StringUtils.translate("litematica.gui.label.area_editor.selection_name"));
        y += 13;
    ...
    protected int addSubRegionFields(int x, int y) {
        ...
        this.addLabel(
                ...
                16,
                0xFFFFFFFF,
                GuiBase.TXT_BOLD + StringUtils.translate("litematica.gui.label.area_editor.sub_regions", str));
    ...
    protected void addRenderingDisabledWarning(int x, int y) {
        if (!Configs.Visuals.ENABLE_AREA_SELECTION_RENDERING.getBooleanValue()) {
            ...
            String str = StringUtils.translate(
                    "litematica.warning.area_editor.area_rendering_disabled", configName, hotkeyName, hotkeyVal);
    ...
    protected void createCoordinateInput(int x, int y, int width, CoordinateType coordType, Corner corner) {
        ...
        y += 2;
        BlockPos pos = corner == Corner.NONE
                ? this.selection.getEffectiveOrigin()
                : this.getBox().getPosition(corner);
        String text = "";
        ButtonListener.Type type = switch (coordType) {
	        case X -> {
		        text = String.valueOf(pos.getX());
		        yield ButtonListener.Type.NUDGE_COORD_X;
	        }
	        case Y -> {
		        text = String.valueOf(pos.getY());
		        yield ButtonListener.Type.NUDGE_COORD_Y;
	        }
	        case Z -> {
		        text = String.valueOf(pos.getZ());
		        yield ButtonListener.Type.NUDGE_COORD_Z;
	        }
        };
    ...
    protected void createCoordinateButton(
            int x, int y, Corner corner, CoordinateType coordType, ButtonListener.Type type) {
    ...
    protected void updateCheckBoxes() {
        ...
        if (this.checkBoxCorner1 != null) {
            boolean checked = this.selection.getSelectedSubRegionBox() != null
                    && this.selection.getSelectedSubRegionBox().getSelectedCorner() == Corner.CORNER_1;
            this.checkBoxCorner1.setChecked(checked, false);
        }

        if (this.checkBoxCorner2 != null) {
            boolean checked = this.selection.getSelectedSubRegionBox() != null
                    && this.selection.getSelectedSubRegionBox().getSelectedCorner() == Corner.CORNER_2;
    ...
    protected static class ButtonListener implements IButtonActionListener {
        ...
        public ButtonListener(
                Type type,
                @Nullable Corner corner,
                @Nullable CoordinateType coordinateType,
                GuiAreaSelectionEditorNormal parent) {
            this.type = type;
            this.corner = corner;
        ...
        public void actionPerformedWithButton(ButtonBase button, int mouseButton) {
            ...
            switch (this.type) {
                case NUDGE_COORD_X, NUDGE_COORD_Y, NUDGE_COORD_Z:
	            ...
	            case CHANGE_SELECTION_MODE:
                    ...
                    if (newMode == SelectionMode.NORMAL && !manager.hasNormalSelection()) {
                        this.parent.addMessage(
                                MessageType.WARNING, "litematica.error.area_editor.switch_mode.no_selection");
                    } else {
                ...
                case ANALYZE_AREA: {
                    ...
                    GuiBase.openGui(gui);
                    list
                            .reCreateMaterialList(); // This is after changing the GUI, so that the task message goes to
                                                     // the new GUI
                    return;
                }
                ...
                case CREATE_SUB_REGION: {
                    GuiTextInput gui = new GuiTextInput(
                            512,
                            "litematica.gui.title.area_editor.sub_region_name",
                            "",
                            null,
                            new SubRegionCreator(this.parent));
    ...
    protected WidgetListSelectionSubRegions createListWidget(int listX, int listY) {
        return new WidgetListSelectionSubRegions(
                listX, listY, this.getBrowserWidth(), this.getBrowserHeight(), this.selection, this);
    }
}

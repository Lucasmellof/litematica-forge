...
public class WidgetListSchematicPlacements extends WidgetListBase<SchematicPlacement, WidgetSchematicPlacement> {
    ...
    protected List<String> getEntryStringsForFilter(SchematicPlacement entry) {
        if (entry.getSchematic().getFile() != null) {
            String fileName = FileUtils.getNameWithoutExtension(
                    entry.getSchematic().getFile().getName().toLowerCase());
    ...
    @Override
    protected WidgetSchematicPlacement createListEntryWidget(
            int x, int y, int listIndex, boolean isOdd, SchematicPlacement entry) {
        return new WidgetSchematicPlacement(

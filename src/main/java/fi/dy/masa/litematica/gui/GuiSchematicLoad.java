...
public class GuiSchematicLoad extends GuiSchematicBrowserBase {
    ...
    public void initGui() {
        ...
        WidgetCheckBox checkbox =
                new WidgetCheckBox(x, y, Icons.CHECKBOX_UNSELECTED, Icons.CHECKBOX_SELECTED, label, hover);
    ...
    private int createButton(int x, int y, int width, ButtonListener.Type type) {
        ...
        if (type == ButtonListener.Type.MATERIAL_LIST) {
            button.setHoverStrings(
                    StringUtils.translate("litematica.gui.button.hover.material_list_shift_to_select_sub_regions"));
        }
    ...
    private static class ButtonListener implements IButtonActionListener {
        ...
        public void actionPerformedWithButton(ButtonBase button, int mouseButton) {
            ...
            if (fileType == FileType.LITEMATICA_SCHEMATIC) {
                ...
                schematic = WorldUtils.convertSchematicaSchematicToLitematicaSchematic(
                        entry.getDirectory(), entry.getName(), false, this.gui);
                warnType = true;
                ...
                this.gui.addMessage(
                        MessageType.ERROR, "litematica.error.schematic_load.unsupported_type", file.getName());
            }
            ...
            if (schematic != null) {
                if (this.type == Type.LOAD_SCHEMATIC) {
                    ...
                    this.gui.addMessage(
                            MessageType.SUCCESS, "litematica.info.schematic_load.schematic_loaded", file.getName());
                    ...
                    if (GuiBase.isShiftDown()) {
                        ...
                        GuiStringListSelection gui =
                                new GuiStringListSelection(schematic.getAreas().keySet(), creator);
                        gui.setTitle(StringUtils.translate(
                                "litematica.gui.title.material_list.select_schematic_regions",
                                schematic.getMetadata().getName()));
                        gui.setParent(GuiUtils.getCurrentScreen());
                        GuiBase.openGui(gui);
                    } else {
                        MaterialListSchematic materialList = new MaterialListSchematic(schematic, true);
                        DataManager.setMaterialList(
                                materialList); // Remember the last opened material list for the hotkey to (re-) open it
                ...
                if (warnType) {
                    InfoUtils.showGuiOrInGameMessage(
                            MessageType.WARNING, 15000, "litematica.message.warn.schematic_load_non_litematica");
                }
            }
        }

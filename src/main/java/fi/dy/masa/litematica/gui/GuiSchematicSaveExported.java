...
public class GuiSchematicSaveExported extends GuiSchematicSaveBase {
    ...
    public GuiSchematicSaveExported(
            DirectoryEntryType type, File dirSource, String inputFileName, ExportType exportType) {
        super(null);
        ...
        this.title = StringUtils.translate(
                "litematica.gui.title.save_exported_schematic", exportType.getDisplayName(), inputFileName);
        ...
        public void actionPerformedWithButton(ButtonBase button, int mouseButton) {
            if (this.type == ButtonType.SAVE) {
                ...
                if (!dir.isDirectory()) {
                    this.gui.addMessage(
                            MessageType.ERROR,
                            "litematica.error.schematic_save.invalid_directory",
                            dir.getAbsolutePath());
                    return;
                }
                ...
                if (fileName.isEmpty()) {
                    this.gui.addMessage(
                            MessageType.ERROR, "litematica.error.schematic_save.invalid_schematic_name", fileName);
                    return;
                }
                ...
                if (this.gui.type == DirectoryEntryType.FILE) {
                    ...
                    if (fileType == FileType.LITEMATICA_SCHEMATIC) {
                        if (this.gui.exportType == ExportType.SCHEMATIC) {
                            if (WorldUtils.convertLitematicaSchematicToSchematicaSchematic(
                                    inDir, inFile, dir, fileName, ignoreEntities, override, this.gui)) {
                                this.gui.addMessage(
                                        MessageType.SUCCESS, "litematica.message.schematic_exported_as", fileName);
                                this.gui.getListWidget().refreshEntries();
                            }
                        } else if (this.gui.exportType == ExportType.VANILLA) {
                            if (WorldUtils.convertLitematicaSchematicToVanillaStructure(
                                    inDir, inFile, dir, fileName, ignoreEntities, override, this.gui)) {
                                this.gui.addMessage(
                                        MessageType.SUCCESS, "litematica.message.schematic_exported_as", fileName);
                                this.gui.getListWidget().refreshEntries();
                ...
                this.gui.addMessage(
                        MessageType.ERROR,
                        "litematica.error.schematic_export.unsupported_type",
                        this.gui.inputFileName);
            }
        }
    }
}

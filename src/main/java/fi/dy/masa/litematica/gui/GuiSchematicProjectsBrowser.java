...
public class GuiSchematicProjectsBrowser
        extends GuiListBase<DirectoryEntry, WidgetDirectoryEntry, WidgetSchematicProjectBrowser>
                    ...
                    if (DataManager.getSchematicProjectsManager().openProject(entry.getFullPath())) {
                        ...
                        if (project != null) {
                            ...
                            InfoUtils.showGuiOrInGameMessage(
                                    MessageType.SUCCESS, "litematica.message.schematic_projects.project_loaded", name);
                        } else {
                            InfoUtils.showGuiOrInGameMessage(
                                    MessageType.ERROR, "litematica.error.schematic_projects.failed_to_load_project");
                        }
                    }
                }
            } else if (this.type == Type.CREATE_PROJECT) {
                ProjectCreator creator =
                        new ProjectCreator(this.gui.getListWidget().getCurrentDirectory(), this.gui);
                GuiTextInput gui = new GuiTextInput(
                        256, "litematica.gui.title.create_schematic_project", "", GuiUtils.getCurrentScreen(), creator);
                GuiBase.openGui(gui);
    ...
    private static class ProjectCreator implements IStringConsumerFeedback {
        ...
        public boolean setString(String projectName) {
            ...
            if (!file.exists()) {
                ...
                this.gui.addMessage(
                        MessageType.SUCCESS, "litematica.message.schematic_projects.project_created", projectName);
                return true;
            }

            InfoUtils.showGuiOrInGameMessage(
                    MessageType.ERROR, "litematica.error.schematic_projects.project_already_exists", projectName);

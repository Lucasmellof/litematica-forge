...
public class WidgetSchematicEntry extends WidgetListEntryBase<LitematicaSchematic> {
    ...
    public void postRenderHovered(int mouseX, int mouseY, boolean selected, PoseStack matrixStack) {
        ...
        if (this.schematic.getMetadata().wasModifiedSinceSaved()
            ...
            String str = WidgetFileBrowserBase.DATE_FORMAT.format(
                    new Date(this.schematic.getMetadata().getTimeModified()));
            List<String> strs =
                    ImmutableList.of(StringUtils.translate("litematica.gui.label.loaded_schematic.modified_on", str));

...
public class WidgetSchematicProjectBrowser extends WidgetFileBrowserBase implements ISelectionListener<DirectoryEntry> {
    ...
    public WidgetSchematicProjectBrowser(
            int x, int y, int width, int height, ISelectionListener<DirectoryEntry> selectionListener) {
        super(
                x,
                y,
                width,
        ...
        if (entry != null) {
            if (entry.getType() == DirectoryEntryType.FILE) {
                this.selectedProject =
                        DataManager.getSchematicProjectsManager().loadProjectFromFile(entry.getFullPath(), false);
            } else {
    ...
    protected void drawAdditionalContents(int mouseX, int mouseY, PoseStack matrixStack) {
        ...
        if (project != null) {
            ...
            str = StringUtils.translate(
                    "litematica.gui.label.schematic_projects.version", strVer, w + project.getVersionCount() + r);
            this.drawString(matrixStack, str, x, y, color);
            y += 12;

...
public class WidgetListSchematicVerificationResults
    ...
    @Override
    protected WidgetSchematicVerificationResult createHeaderWidget(
            int x, int y, int listIndexStart, int usableHeight, int usedHeight) {
        ...
        String strExpected =
                TXT_BOLD + StringUtils.translate(WidgetSchematicVerificationResult.HEADER_EXPECTED) + TXT_RST;
        BlockMismatchEntry entry;

        if (type != MismatchType.CORRECT_STATE) {
            String strFound = TXT_WHITE
                    + TXT_BOLD
                    + StringUtils.translate(WidgetSchematicVerificationResult.HEADER_FOUND)
                    + TXT_RST;
    ...
    private void addEntriesForType(MismatchType type) {
        ...
        if (type == MismatchType.CORRECT_STATE) {
            Object2IntOpenHashMap<BlockState> counts = this.guiSchematicVerifier
                    .getPlacement()
            ...
            for (ItemType itemType : itemCounts.keySet()) {
                ...
                BlockMismatch mismatch =
                        new BlockMismatch(MismatchType.CORRECT_STATE, state, state, itemCounts.getInt(itemType));
                list.add(mismatch);
            }
        } else {
            list = this.guiSchematicVerifier
                    .getPlacement()
                    .getSchematicVerifier()
                    .getMismatchOverviewFor(type);
        }
    ...
    @Override
    protected WidgetSchematicVerificationResult createListEntryWidget(
            int x, int y, int listIndex, boolean isOdd, BlockMismatchEntry entry) {
        return new WidgetSchematicVerificationResult(
                x,
                y,

...
public abstract class GuiSchematicSaveBase extends GuiSchematicBrowserBase
    ...
    public void initGui() {
        ...
        this.textField.moveCursorTo(pos);
        this.textField.changeFocus(focused);
        ...
        if (this.lastText.isEmpty()) {
            if (entry != null
                    && entry.getType() != DirectoryEntryType.DIRECTORY
                    && entry.getType() != DirectoryEntryType.INVALID) {
        ...
        this.checkboxIgnoreEntities =
                new WidgetCheckBox(x, y + 24, Icons.CHECKBOX_UNSELECTED, Icons.CHECKBOX_SELECTED, str);
        this.addWidget(this.checkboxIgnoreEntities);

        this.checkboxVisibleOnly = new WidgetCheckBox(
                12,
                y + 24,
                Icons.CHECKBOX_UNSELECTED,
                Icons.CHECKBOX_SELECTED,
    ...
    private int createButton(int x, int y, ButtonType type) {
        ...
        if (type == ButtonType.SAVE) {
            button = new ButtonGeneric(
                    x, y, width, 20, label, "litematica.gui.label.schematic_save.hoverinfo.hold_shift_to_overwrite");
        } else {
    ...
    public boolean onKeyTyped(int keyCode, int scanCode, int modifiers) {
        if (this.textField.keyPressed(keyCode, scanCode, modifiers)) {
            ...
            this.textField.changeFocus(!this.textField.isFocused());

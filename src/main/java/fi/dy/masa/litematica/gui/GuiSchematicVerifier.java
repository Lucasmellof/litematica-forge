...
public class GuiSchematicVerifier
        extends GuiListBase<
                BlockMismatchEntry, WidgetSchematicVerificationResult, WidgetListSchematicVerificationResults>
    ...
    public void initGui() {
        ...
        if (this.verifier.isActive()) {
            String str = StringUtils.translate(
                    "litematica.gui.label.schematic_verifier.status.verifying",
                    this.verifier.getUnseenChunks(),
                    this.verifier.getTotalChunks());
            ...
            String str =
                    StringUtils.translate("litematica.gui.label.schematic_verifier.status.done_errors", wb, ws, m, e);
            this.addLabel(12, y, 100, 12, 0xFFF0F0F0, str);
    ...
    private int createButton(int x, int y, int width, ButtonListener.Type type) {
        ...
        switch (type) {
            ...
            case RESET_IGNORED:
                ...
                enabled = !this.verifier.getIgnoredMismatches().isEmpty();
                break;

            case TOGGLE_INFO_HUD: {
                boolean val = InfoHud.getInstance().isEnabled() && this.verifier.getShouldRenderText(RenderPhase.POST);
                String str = (val ? TXT_GREEN : TXT_RED)
                        + StringUtils.translate("litematica.message.value." + (val ? "on" : "off"))
                        + TXT_RST;
    ...
    public void onSelectionChange(@Nullable BlockMismatchEntry entry) {
        if (entry != null) {
            ...
            if (!Configs.InfoOverlays.VERIFIER_OVERLAY_ENABLED.getBooleanValue()) {
                ...
                String hotkeyVal =
                        Hotkeys.TOGGLE_VERIFIER_OVERLAY_RENDERING.getKeybind().getKeysDisplayString();
                InfoUtils.showGuiOrInGameMessage(
                        MessageType.WARNING,
                        "litematica.message.warn.schematic_verifier.overlay_disabled",
                        name,
                        hotkeyName,
                        hotkeyVal);
            }
    ...
    protected WidgetListSchematicVerificationResults createListWidget(int listX, int listY) {
        return new WidgetListSchematicVerificationResults(
                ...
                case START:
                    ...
                    if (world != null) {
                        ...
                        this.parent.addMessage(
                                MessageType.ERROR, "litematica.error.generic.schematic_world_not_loaded");
                    }

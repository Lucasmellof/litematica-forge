...
public class WidgetSelectionSubRegion extends WidgetListEntryBase<String> {
    ...
    public WidgetSelectionSubRegion(
            int x,
            int y,
            int width,
            int height,
            ...
            WidgetListSelectionSubRegions parent) {
    ...
    private int createButton(int x, int y, ButtonListener.ButtonType type) {
        return this.addButton(new ButtonGeneric(x, y, -1, true, type.getDisplayName()), new ButtonListener(type, this))
                        .getX()
                - 1;
    }
        ...
        if (this.box != null) {
            ...
            if (pos1 != null && pos2 != null) {
                ...
                text.add(String.format(
                        "%s: %d x %d x %d", str, Math.abs(size.getX()), Math.abs(size.getY()), Math.abs(size.getZ())));
            }
        }

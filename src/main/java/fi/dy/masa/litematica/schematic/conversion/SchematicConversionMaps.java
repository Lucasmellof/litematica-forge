...
public class SchematicConversionMaps {
    private static final Object2IntOpenHashMap<String> OLD_BLOCK_NAME_TO_SHIFTED_BLOCK_ID =
            DataFixUtils.make(new Object2IntOpenHashMap<>(), (map) -> {
                map.defaultReturnValue(-1);
            });
    private static final Int2ObjectOpenHashMap<String> ID_META_TO_UPDATED_NAME = new Int2ObjectOpenHashMap<>();
    private static final Object2IntOpenHashMap<BlockState> BLOCKSTATE_TO_ID_META =
            DataFixUtils.make(new Object2IntOpenHashMap<>(), (map) -> {
                map.defaultReturnValue(-1);
            });
        ...
        for (ConversionData data : CACHED_DATA) {
            try {
                ...
                Litematica.logger.warn(
                        "addEntry(): Exception while adding blockstate conversion map entry for ID '{}' (fixed state: '{}')",
                        data.idMeta,
                        data.newStateString,
                        e);
            }
    ...
    private static void addOverrides() {
        ...
        ID_META_TO_BLOCKSTATE.put(
                idOldLog | 0, Blocks.OAK_WOOD.defaultBlockState().setValue(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(
                idOldLog | 1,
                Blocks.SPRUCE_WOOD.defaultBlockState().setValue(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(
                idOldLog | 2,
                Blocks.BIRCH_WOOD.defaultBlockState().setValue(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(
                idOldLog | 3,
                Blocks.JUNGLE_WOOD.defaultBlockState().setValue(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(
                idNewLog | 0,
                Blocks.ACACIA_WOOD.defaultBlockState().setValue(RotatedPillarBlock.AXIS, Direction.Axis.Y));
        ID_META_TO_BLOCKSTATE.put(
                idNewLog | 1,
                Blocks.DARK_OAK_WOOD.defaultBlockState().setValue(RotatedPillarBlock.AXIS, Direction.Axis.Y));
    ...
    private static void addIdMetaToBlockState(int idMeta, CompoundTag newStateTag, String... oldStateStrings) {
        try {
            ...
            Litematica.logger.warn(
                    "addIdMetaToBlockState(): Exception while adding blockstate conversion map entry for ID '{}'",
                    idMeta,
                    e);
        }
    }
    ...
    private static void addOldStateToNewState(CompoundTag newStateTagIn, String... oldStateStrings) {
        try {
            ...
            Litematica.logger.warn(
                    "addOldStateToNewState(): Exception while adding new blockstate to old blockstate conversion map entry for '{}'",
                    newStateTagIn,
                    e);
        }
    }
    ...
    public static String updateBlockName(String oldName) {
        ...
        return Minecraft.getInstance()
                .getFixerUpper()
                .update(
                        References.BLOCK_NAME,
                        new Dynamic<>(NbtOps.INSTANCE, tagStr),
                        1139,
                        LitematicaSchematic.MINECRAFT_DATA_VERSION)
                .getValue()
                .getAsString();
    }

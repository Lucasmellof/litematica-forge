...
public class SchematicProject {
        ...
        if (!newFile.exists()) {
            try {
                ...
                InfoUtils.showGuiOrInGameMessage(
                        MessageType.ERROR,
                        "litematica.error.schematic_projects.failed_to_rename_project_file_exception",
                        newFile.getAbsolutePath());
            }
        } else {
            InfoUtils.showGuiOrInGameMessage(
                    MessageType.ERROR,
                    "litematica.error.schematic_projects.failed_to_rename_project_file_exists",
                    name);
        }
    }
        ...
        if (version != null) {
            if (this.currentVersionId != this.lastCheckedOutVersion) {
                ...
                LitematicaSchematic schematic =
                        LitematicaSchematic.createFromFile(this.directory, version.getFileName());

                if (schematic != null) {
                    BlockPos areaPosition = this.origin.offset(version.getAreaOffset());
                    this.currentPlacement =
                            SchematicPlacement.createFor(schematic, areaPosition, version.getName(), true, true);
                    ...
                    if (time != version.getTimeStamp()) {
                        version = new SchematicVersion(
                                version.getName(),
                                version.getFileName(),
                                version.getAreaOffset(),
                                version.getVersion(),
                                time);
                    ...
                    InfoUtils.showGuiOrInGameMessage(
                            MessageType.ERROR, "litematica.error.schematic_projects.failed_to_load_schematic");
                }
    ...
    public void pasteToWorld() {
        if (this.currentPlacement != null) {
            ...
            ToolUtils.deleteSelectionVolumes(
                    this.lastSeenArea,
                    true,
                    () -> {
                        DataManager.getSchematicPlacementManager()
                                .pastePlacementToWorld(this.currentPlacement, false, mc);
                    },
                    mc);
        }
    }
    ...
    public boolean commitNewVersion(String name) {
        if (this.checkCanSaveOrPrintError()) {
            ...
            TaskSaveSchematic task =
                    new TaskSaveSchematic(this.directory, fileName, schematic, selection.copy(), info, false);
    ...
    public static SchematicProject fromJson(JsonObject obj, File projectFile, boolean createPlacement) {
        ...
        if (JsonUtils.hasString(obj, "name") && JsonUtils.hasInteger(obj, "current_version_id") && origin != null) {
            ...
            if (JsonUtils.hasObject(obj, "selection_simple")) {
                project.selectionSimple =
                        AreaSelectionSimple.fromJson(JsonUtils.getNestedObject(obj, "selection_simple", false));
            }
    ...
    boolean checkCanSaveOrPrintError() {
        if (this.saveInProgress) {
            InfoUtils.showGuiOrInGameMessage(
                    MessageType.ERROR, "litematica.error.schematic_projects.save_already_in_progress");
            return false;
        }

        if (this.directory == null || !this.directory.exists() || !this.directory.isDirectory()) {
            InfoUtils.showGuiOrInGameMessage(
                    MessageType.ERROR, "litematica.error.schematic_projects.invalid_project_directory");
            return false;
        }

        if (this.getSelection().getAllSubRegionBoxes().isEmpty()) {
    ...
    private class SaveCompletionListener implements ICompletionListener {
        ...
        private void saveVersion() {
            SchematicVersion version = new SchematicVersion(
                    this.name, this.fileName, this.areaOffset, this.version, System.currentTimeMillis());
            ...
            InfoUtils.showGuiOrInGameMessage(
                    MessageType.SUCCESS,
                    "litematica.message.schematic_projects.version_saved",
                    this.version,
                    this.name);
        }
    }
